...
function createWindow() {
  ...
  mainWindow.on('closed', () => {
    mainWindow = null;
    if (printerStatusCheckInterval) clearInterval(printerStatusCheckInterval);
  });
}
...
ipcMain.handle('print-receipt', async (_event, escPosData) => {
  try {
    if (!lanPrinter) throw new Error('Printer service not initialized');
    const buffer = Buffer.isBuffer(escPosData) ? escPosData : Buffer.from(escPosData);
...
ipcMain.handle('get-printer-status', async () => {
  if (!lanPrinter) return { isConnected: false, ip: 'Not initialized', port: 0 };
